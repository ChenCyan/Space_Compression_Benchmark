"""Split-plot suite for Berlin-Dense dataset — uses self-trained HyCASS weights."""
...
# Data sources: all from berlin-dense dataset, self-trained HyCASS
...
OUT = f"{DATA_DIR}/results/plots_berlin_dense"
...
STYLE = {
    ...
    "hycass":       dict(c="#377eb8",m="^",label="HyCASS (Dense)",   ls="-."),
}
...
def load_data():
    ...
    rows = load_csv(CSV_DENSE_BENCH, rows)  # self-trained HyCASS (dense)
...
plot_panel(ax,gl,"throughput_mbs","Throughput (MB/s)",log_y=True,
           title="Lossless — CR vs Throughput (Dense)",
...
for idx,(key,ylab,logy,title) in enumerate([
    ("throughput_mbs","Throughput (MB/s)",True,"Near-Lossless — CR vs Throughput (Dense)"),
    ("psnr","PSNR (dB)",False,"Near-Lossless — CR vs PSNR (Dense)"),
    ("pae","PAE (DN)",True,"Near-Lossless — CR vs PAE (Dense)"),
], start=2):
...
# --- 5-7. Lossy (includes self-trained HyCASS) ---
lf = ["jpeg2000","klt_dwt_nc28","klt_dwt_nc56","klt_dwt_nc111","hycass"]
gy = group_rows(rows, lf, restrict_to_rows=lossy)
for idx,(key,ylab,logy,title) in enumerate([
    ("psnr","PSNR (dB)",False,"Lossy — CR vs PSNR (Dense)"),
    ("pae","PAE (DN)",True,"Lossy — CR vs PAE (Dense)"),
    ("throughput_mbs","Throughput (MB/s)",True,"Lossy — CR vs Throughput (Dense)"),
], start=5):
    fig,ax=plt.subplots(figsize=(6.5,4.5))
    fn = {"psnr":f_psnr}.get(key)
    plot_panel(ax,gy,key,ylab,log_y=logy,title=title,map_fn=fn,
               label_override={"jpeg2000":"JPEG2000$_{MCT}$ (cr=2–100)",
